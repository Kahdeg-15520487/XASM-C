...
static void emitConstant(Value value) {
  emitBytes(OP_YEET, makeConstant(value));
}
...
ParseRule rules[] = {
    ...
    [TOKEN_INT] = {number_int, NULL, PREC_NONE},
...
static void function(FunctionType type) {
  ...
  if (!check(TOKEN_RIGHT_PAREN)) {
    do {
      ...
      uint8_t paramConstant =
          parseVariable("Expect parameter name.", MULTIPLE_ASSIGN);
  ...
  emitBytes(OP_YEET, makeConstant(OBJ_VAL(function)));
}

static void funDeclaration() {
  uint8_t global = parseVariable("Expect function name.", MULTIPLE_ASSIGN);
  markInitialized();
...
static void doWhileStatement() {
  ...
  statement();
  consume(TOKEN_WHILE, "Expect \"while\" clause in do while");
